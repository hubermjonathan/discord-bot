...
private val commandData = Commands.slash(name, "upload an emoji")
...
class UploadEmojiCommand(context: Context) : Command(name, commandData, context) {
    ...
    override fun execute(event: SlashCommandInteractionEvent) {
        ...
        if (fileExtension == null || fileExtension !in allowedFileExtensions) {
            throw IllegalArgumentException("emoji upload must be one of $allowedFileExtensions")
        }

        val emojiName = attachment.fileName.removeSuffix(".$fileExtension")

        val emoji = guild.createEmoji(emojiName, attachment.proxy.downloadAsIcon().get())
            .complete()
